...
planes %>% 
  count(tailnum) %>% 
  filter(n > 1) #uniquely identified
...
flights %>% 
  filter(origin = "EWR", month == 1, day == 1, flight == 688) #??fuck this for

# itâ€™s sometimes useful to add one with mutate() and row_number(). 
...
flights2 %>% left_join(airports, by = c("dest" = "faa"))  # notice the syntax!! #correct
...
#   If you just look for variables without thinking about what they mean, you might get (un)lucky and find a combination thatâ€™s unique in your current data but the relationship might not be true in general.
# 

# Check that none of the variables in the primary key are missing. 
#   If a value is missing then it canâ€™t identify an observation!
#   
# Check that your foreign keys match primary keys in another table. 
#   The best way to do this is with an anti_join(). 
#   Itâ€™s common for keys not to match because of data entry errors. 
#   Fixing these is often a lot of work.
# 
# If you do have missing keys, youâ€™ll need to be thoughtful about 
